...
def cal_distance(a, b, A_padding, B, p_size):
    ...
    # difference between patch A and patch B
    temp = patch_b - patch_a

    num = np.sum(1 - np.int32(np.isnan(temp)))  # count for the number of not-nans
...
def initialization(A, B, p_size):
    ...
    f = np.zeros([A_h, A_w], dtype=object)      # I think this is random offset vector
    dist = np.zeros([A_h, A_w])                 # distance between patch-A and patch-B
    for i in range(A_h):
        for j in range(A_w):
            a = np.array([i, j])
...
def NNS(img, ref, p_size, itr):
    ...
    for itr in range(1, itr+1):
        if itr % 2 == 0:    # even
            for i in range(A_h - 1, -1, -1):        # Offsets are examined in reverse scan order
                for j in range(A_w - 1, -1, -1):    # from right to left, bottom to up
                    a = np.array([i, j])
                    propagation(f, a, dist, img_padding, ref, p_size, False)
                    random_search(f, a, dist, img_padding, ref, p_size)
        else:               # odd
            for i in range(A_h):                    # Offsets are examined in scan order
                for j in range(A_w):                # from left to right, top to bottom
                    a = np.array([i, j])
